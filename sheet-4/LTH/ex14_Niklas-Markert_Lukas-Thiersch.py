...
def fct(length, counter):           # function fct is defined with parameters
    ...
    if length > 5:                  # check if length is greater 5
        ...
        turtle.backward(length)     # the turtle goes backwards lenght distance
                                    # --> it is now at the point, where
                                    # it was before fct

        logging.debug("Function fct(length, counter) is finished with length = "
                      + str(length) + " and counter = " + str(counter) + ".")


def main():
    logging.debug("Function main() called. No args.")
    counter, length = "a", 85         # set counter to 0 and length to 85
...
# not a number or float

# Length: The length's type must be a number as turning the turtle left or
# right by a string or boolean value would not make sense and ultimately
